...
class RouterQueryEngineWorkflow(Workflow):
    @step
    async def selector(self, ctx: Context, ev: StartEvent) -> QueryEngineSelectionEvent:
        """
    ...
    async def generate_responses(
        ...
        if len(selected_query_engines.selections) > 1:
            ...
            query_engine = query_engines[selected_query_engines.selections[0].index]
    ...
    async def acombine_responses(
        ...
        for response in responses:
            if isinstance(response, (AsyncStreamingResponse, PydanticResponse)):
...
if __name__ == "__main__":
    asyncio.run(run_router_workflow())